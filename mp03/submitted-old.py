...
def k_nearest_neighbors(image, train_images, train_labels, k):
    ...
    def acceptance(distance, label, smallest_distances, labels):
        if np.any(smallest_distances == -1):
            return True, np.where(smallest_distances == -1)[0][0]
        elif np.any(smallest_distances > distance):
            return True, np.where(smallest_distances > distance)[0][0]
        elif np.any(smallest_distances == distance) and label is False:
            for idx in np.where(smallest_distances == distance)[0][0]:
                if labels[idx] is True:
                    return True, idx
        else:
            return False, -1

    neighbors = np.zeros((k, np.shape(train_images[0])[0]))
    smallest_distances = np.ones(k) * -1
    ...
    for img_idx in range(len(train_images)):
        ...
        should_accept, idx = acceptance(distance, train_label,
                                        smallest_distances, labels)
        if should_accept:
...
def confusion_matrix(hypotheses, references):
    ...
    precision = confusions[1][1] / np.sum(confusions[:, 1])
    recall = confusions[1][1] / (confusions[1][1] + confusions[0][0])
