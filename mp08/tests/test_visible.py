...
def test_synthetic():
    ...
    try:
        ...
        if accuracy < 0.3:
            print(
                "Synthetic test: there seems to be major bugs in your Viterbi algorithm"
            )
            print("penalty multiplier: 0.2")
            return 0.2
        elif 0.3 <= accuracy < 0.9:
            print(
                "Synthetic test: did you backtrace properly in your Viterbi algorithm?"
            )
...
class TestMP4(unittest.TestCase):
    ...
    def test_brown_baseline(self):
        ...
        try:
            _, time_spend, accuracy, multi_tag_accuracy, unseen_words_accuracy = runner(
                algorithm, train_set, test_set
            )
            self.assertLessEqual(
                time_spend,
                max_time_spend,
                "The {0} should run in less than {1} secs.".format(
                    name, max_time_spend
                ),
            )
            self.assertGreaterEqual(
                accuracy,
                min_accuracy,
                "The {0} accuracy should be at least {1}.".format(name, min_accuracy),
            )
            self.assertGreaterEqual(
                multi_tag_accuracy,
                min_multi_tag_accuracy,
                "The {0} multi-tag accuracy should be at least {1}.".format(
                    name, min_multi_tag_accuracy
                ),
            )
            self.assertGreaterEqual(
                unseen_words_accuracy,
                min_unseen_words_accuracy,
                "The {0} unseen word accuracy should be at least {1}.".format(
                    name, min_unseen_words_accuracy
                ),
            )
        except Exception as e:
            print(e)
    ...
    def test_brown_viterbi(self, set_score=None):
        ...
        level_1 = [
            ...
            0.87,
            0.17,
        ]  # overall_accuracy, multi_tag_accuracy, unseen_words_accuracy
        ...
        if (
            accuracy >= level_1[0]
            and multi_tag_accuracy >= level_1[1]
            and unseen_words_accuracy >= level_1[2]
        ):
            total_score += 10
            print(
                "+10 points for accuracy, multi_tag_accuracy, unseen_words_accuracy above {0} respectively.".format(
                    level_1
                )
            )
        else:
            print(
                "The accuracy, multi_tag_accuracy, unseen_words_accuracy should be at least {0} respectively for 5 points.".format(
                    level_1
                )
            )
        ...
        if (
            accuracy >= level_2[0]
            and multi_tag_accuracy >= level_2[1]
            and unseen_words_accuracy >= level_2[2]
        ):
            total_score += 10
            print(
                "+10 points for accuracy, multi_tag_accuracy, unseen_words_accuracy above {0} respectively.".format(
            ...
            print(
                "The accuracy, multi_tag_accuracy, unseen_words_accuracy should be at least {0} respectively for 15 points.".format(
                    level_2
                )
            )
        ...
        if (
            accuracy >= level_3[0]
            and multi_tag_accuracy >= level_3[1]
            and unseen_words_accuracy >= level_3[2]
        ):
            total_score += 10
            print(
                "+10 points for accuracy, multi_tag_accuracy, unseen_words_accuracy above {0} respectively.".format(
            ...
            print(
                "The accuracy, multi_tag_accuracy, unseen_words_accuracy should be at least {0} respectively for 20 points.".format(
                    level_3
                )
            )
...
def runner(algorithm, train_set, test_set):
    try:
        ...
        (
            multi_tag_accuracy,
            unseen_words_accuracy,
        ) = utils.specialword_accuracies(train_set, students_answer, test_set)
