...
def evaluate_accuracies(predicted_sentences, tag_sentences):
    ...
    """
    assert len(predicted_sentences) == len(
        tag_sentences
    ), "The number of predicted sentence {} does not match the true number {}".format(
    ...
    for pred_sentence, tag_sentence in zip(predicted_sentences, tag_sentences):
        assert len(pred_sentence) == len(
            tag_sentence
        ), "The predicted sentence length {} does not match the true length {}".format(
            len(pred_sentence), len(tag_sentence)
        )
        for pred_wordtag, real_wordtag in zip(pred_sentence, tag_sentence):
            assert (
                pred_wordtag[0] == real_wordtag[0]
            ), "The predicted sentence WORDS do not match with the original sentence, you should only be predicting the tags"
            word = pred_wordtag[0]
...
def topk_wordtagcounter(wordtagcounter, k):
    top_items = sorted(
        wordtagcounter.items(), key=lambda item: sum(item[1].values()), reverse=True
    )[:k]
...
def get_word_tag_statistics(data_set):
    ...
    return word_set, set(
        map(
            lambda elem: elem[0],
            filter(lambda elem: len(elem[1]) > 1, word_tags.items()),
        )
    )
