...
class PongGame:
    def __init__(
        ...
        if learner == None and not (visible):
            raise RuntimeError(
                "Human player can only be used with a visible playing board"
            )
        ...
        if self.visible:
            ...
            self.display = pong_display.PongBoard(
                self.game_w, self.game_h, self.paddle_w, self.paddle_h
            )
    ...
    def quantize_state(self, state):
        ...
        """
        return [
            int(state[0] * self.state_quantization[0] / self.game_w),
            int(state[1] * self.state_quantization[1] / self.game_h),
            int(
                (state[2] + self.max_ball_speed)
                * self.state_quantization[2]
                / (2 * self.max_ball_speed + 1)
            ),
            int(
                (state[3] + self.max_ball_speed)
                * self.state_quantization[3]
                / (2 * self.max_ball_speed + 1)
            ),
    ...
    def update(self, state, paddle_v):
        ...
        paddle_y = max(
            self.half_pad_h,
            min(self.game_h - self.half_pad_h, paddle_y + paddle_v),
        )
        ...
        ball_y = max(
            self.radius, min(self.game_h - self.radius, ball_y + ball_vy)
        )
        return [ball_x, ball_y, ball_vx, ball_vy, paddle_y], reward

    def run(
        self, m_rewards=np.inf, m_games=np.inf, m_frames=np.inf, states=[]
    ):
        """
        Run the game.
        @param
        ...
        while (
            n_rewards < m_rewards and n_games < m_games and n_frames < m_frames
        ):
            n_frames += 1
            ...
            if self.visible:
                ...
                self.display.draw_scores(
                    self.n_games, self.score, self.max_score
                )
            ...
            if self.learner == None and self.visible:
                ...
                raise RuntimeError(
                    "pong learner has unrecognized type:", type(self.learner)
                )
            ...
            # learn
            if (
                type(self.learner) == submitted.q_learner
                or type(self.learner) == submitted.deep_q
            ):
                if self.state_quantization:
                    state = self.quantize_state(state)
                    self.learner.learn(
                        state, action, reward, self.quantize_state(newstate)
                    )
                else:
                    self.learner.learn(state, action, reward, newstate)
                if (
                    m_frames < np.inf
                    or (m_rewards < np.inf and reward != 0)
                    or (reward < 0)
                ):
                    q_achieved.append(self.learner.report_q(state))
                    q_states.append(
                        np.array([self.learner.report_q(s) for s in states])
                    )
            state = newstate
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--paddle_speed", default="8", help="How fast should the paddle be?"
    )
    parser.add_argument(
        "--alpha", default="0.05", help="Reinforcement learning rate"
    )
    parser.add_argument(
        "--epsilon", default="0.1", help="Exploration probability"
    )
    parser.add_argument("--gamma", default="0.99", help="discount factor")
    parser.add_argument(
        "--nfirst", default="5", help="number of exploration per state/action"
    )
    parser.add_argument(
        "--state_quantization",
        default="[10,10,2,2,10]",
        help="""
    ...
    if args.player == "q_learning":
        learner = submitted.q_learner(
            alpha, epsilon, gamma, nfirst, state_quantization
        )
