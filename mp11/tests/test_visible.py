...
class TestStep(unittest.TestCase):
    ...
    def test_choose_unexplored_action(self):
        q_learner = submitted.q_learner(
            0.05, 0.05, 0.99, 1, [10, 10, 2, 2, 10]
        )
        actions = []
        for i in range(4):
            actions.append(q_learner.choose_unexplored_action([9, 9, 1, 1, 9]))
        self.assertEqual(
            np.count_nonzero([y == 1 for y in actions[:3]]),
            1,
        ...
        self.assertEqual(
            np.count_nonzero([y == 0 for y in actions[:3]]),
            1,
    ...
    def test_q_local(self):
        q_learner = submitted.q_learner(
            0.05, 0.05, 0.99, 5, [10, 10, 2, 10, 10]
        )
    ...
    def test_exploit(self):
        q_learner1 = submitted.q_learner(
            0.05, 0.05, 0.99, 5, [10, 10, 2, 2, 10]
        )
    ...
    def test_trained_model(self):
        ...
        q_learner = submitted.q_learner(
            0.05, 0.05, 0.99, 5, state_quantization
        )
