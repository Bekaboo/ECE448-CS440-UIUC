...
class PongBoard:
    def __init__(self, game_w, game_h, paddle_w, paddle_h):
        ...
        self.window = pygame.display.set_mode(
            [int(game_w), int(game_h)], 0, 32
        )
        pygame.display.set_caption("Pong")
        self.font = pygame.font.SysFont("Comic Sans MS", 20)
        self.paddle_corners = np.array(
            [
        ...
        self.centerline = np.array(
            [[int(game_w / 2), 0], [int(game_w / 2), int(game_h)]]
        )
    ...
    def draw(self, ball_x, ball_y, paddle_y, ball_radius):
        ...
        pygame.draw.circle(
            self.window, pygame.Color("white"), self.centerpoint, 70, 1
        )

        # Draw the paddle, ball, and score
        paddle_corners = [
            [int(self.paddle_x + c[0]), int(paddle_y + c[1])]
            for c in self.paddle_corners
        ]
        pygame.draw.polygon(
            self.window, pygame.Color("green"), paddle_corners, 0
        )
        pygame.draw.circle(
            self.window,
            pygame.Color("red"),
